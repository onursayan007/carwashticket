...
<template>
  <main class="relative h-dvh overflow-hidden bg-slate-100">
    ...
    <!-- Üst çubuk -->
    <div class="pointer-events-none absolute inset-x-0 top-0 z-10 space-y-3 p-3">
      <div class="pointer-events-auto flex items-center gap-2">
        <div class="flex-1 rounded-full bg-white/95 px-4 py-2.5 shadow-lg backdrop-blur">
          <p class="text-sm font-semibold text-slate-900">Araç yıkama</p>
        ...
        <RouterLink
          :to="{ name: 'wallet' }"
          class="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-white/95 text-lg shadow-lg backdrop-blur"
      ...
      <div class="pointer-events-auto -mx-3 flex gap-2 overflow-x-auto px-3 pb-1">
        <button
          ...
          :key="option.value"
          type="button"
          class="shrink-0 rounded-full px-3.5 py-1.5 text-sm font-medium shadow-md transition"
          :class="
            sort === option.value
    ...
    >
      ...
      >
        ...
        <span class="flex w-full items-center justify-between px-4">
          ...
          <span class="text-xs text-slate-500">{{ expanded ? 'Haritayı gör' : 'Listeyi aç' }}</span>
        </span>
      </button>
        ...
        <ul v-else class="space-y-2">
          <li v-for="station in stations" :key="station.id">
            <button
              ...
              "
              @click="selectedId === station.id ? open(station.id) : selectStation(station.id)"
            >
              ...
              <p
                ...
                class="mt-2 text-xs font-medium text-slate-900"
              >
                Tekrar dokunun → hizmetleri görün
              </p>
            </button>
          </li>
        </ul>
